...
w = 3
h = w  # create a square grid for game
turns_taken = []  # use to keep track of each turn already taken
# use this matrix to test for wins later
game = [[0 for col in range(w)] for row in range(h)]
game_over = False
...
while not game_over:
    ...
    # check for a horizontal win using 'game' matrix
    ...
    # check for a diagonal win if game is still active
    if not game_over:
        diag1 = 1
        for i in range(w):
            diag1 *= game[i][i]
        if diag1 == 1:
            print("Diagonal X wins!\n")
            game_over = True
        elif diag1 == 2**w:
            print("Diagonal Y wins!\n")
            game_over = True

    # check other diagonal for a win
    if not game_over:
        diag2 = 1
        for i in range(w):
            diag2 *= game[i][w - 1 - i]
    ...
    if (
        not game_over and len(turns_taken) == w * h
    ):  # no one has won after the matrix is full
        game_over = True
        print("Game is a draw.  Restart to go again.\n")
    elif not game_over and len(turns_taken) < w * h:
        print("No winner yet...\n")
