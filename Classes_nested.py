...
class HighSchool(School):
    ...
    def get_sportsTeams(self):
        return sorted(self.sportsTeams)

    def __repr__(self):
        return f"We are {self.name} high school with the following sports: {self.get_sportsTeams()}"


joes_school = PrimarySchool("Joe's School", 55, "Be on Time!")
print(joes_school)
tims_high = HighSchool("Tim's High", 3000, ["Tennis", "Bball", "Foot"])
print(tims_high.get_sportsTeams())  # list is sorted in the function 'get_sportsTeams()'
print(tims_high.sportsTeams)  # list as entered.  this is the original instance
print(tims_high)  # called from def __repr__(self) as {self.get_sportsTeams()}
